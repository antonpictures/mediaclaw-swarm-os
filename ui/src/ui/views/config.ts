...
export function renderConfig(props: ConfigProps) {
  ...
  return html`
    <div class="config-layout">
      <!-- NEW macOS Style Sidebar -->
      <aside class="config-sidebar">
        <!-- Optional search integration in sidebar -->
        ${
          formMode === "form"
            ? html`
        ...
        <div class="config-sidebar__nav" role="tablist" aria-label="Settings sections">
          <div class="config-sidebar__group">
            ${topTabs
              ...
              .map(
                (tab) => html`
              <button
                class="config-sidebar__tab ${props.activeSection === tab.key ? "active" : ""}"
                role="tab"
                aria-selected=${props.activeSection === tab.key}
                @click=${() => props.onSectionChange(tab.key)}
                title=${tab.label}
              >
                <div class="config-sidebar__tab-icon">
                  ${getSectionIcon("__appearance__")}
                </div>
                <span>${tab.label}</span>
              </button>
            `,
          ...
          ${visibleCategories.map(
            (cat) => html`
            <div class="config-sidebar__group">
              <div class="config-sidebar__group-title">${cat.label}</div>
              ${cat.sections.map(
                (tab) => html`
                <button
                  class="config-sidebar__tab ${props.activeSection === tab.key ? "active" : ""}"
                  ...
                  title=${tab.label}
                >
                  <div class="config-sidebar__tab-icon config-sidebar__tab-icon--${cat.id}">
                    ${getSectionIcon(tab.key)}
                  </div>
                  <span>${tab.label}</span>
                </button>
              `,
          ...
          ${
            otherCategory
              ? html`
            <div class="config-sidebar__group">
              <div class="config-sidebar__group-title">${otherCategory.label}</div>
              ${otherCategory.sections.map(
                (tab) => html`
                <button
                  class="config-sidebar__tab ${props.activeSection === tab.key ? "active" : ""}"
                  role="tab"
      ...
      <main class="config-main">
        ...
        <!-- The old config-top-tabs are removed now because they are in the sidebar -->
        <div class="config-top-tabs" style="display: none;">
          ${
            formMode === "form"
              ? html`
